...
public class DownloadController : BaseAuthController {
  ...
  public async Task<IActionResult> DownloadFile(string entryId) {
    try {
      ...
      var stream = await _fileUtilities
        .GetRemoteFileStream(_audioStorageSettings.ContainerName, $"{entry.Id}.mp3");
      Response.Headers.Append("Content-Disposition", $"attachment; filename=\"{entry.GetFileDownloadName()}\"");
