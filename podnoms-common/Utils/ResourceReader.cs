  ...
  public static async Task<string> ReadResource(string resourceName, bool minifyContent = false) {
    ...
    if (resourceStream != null) {
      using (var reader = new StreamReader(resourceStream, Encoding.UTF8)) {
        return minifyContent ? reader.MinifyHtmlCode(new Features(new string[] { })) : reader.ReadToEnd();
      }
    }
    ...
    using (var reader = new StreamReader(File.Open($"/app/Resources/{resourceName}", FileMode.Open))) {
      ...
      return minifyContent ? reader.MinifyHtmlCode(new Features(new string[] { })) : await reader.ReadToEndAsync();
    }
  }
}
