...
public static class HtmlUtils {
  public static string FormatLineBreaks(string html) {
    ...
    foreach (var node in doc.DocumentNode.SafeSelectNodes("//comment() | //script | //style | //head")) {
      node.ParentNode.RemoveChild(node);
    }

    //now remove all "meaningless" inline elements like "span"
    foreach (var node in doc.DocumentNode.SafeSelectNodes("//span | //label")) //add "b", "i" if required
    {
      node.ParentNode.ReplaceChild(HtmlNode.CreateNode(node.InnerHtml), node);
    }

    //block-elements - convert to line-breaks
    foreach (var node in doc.DocumentNode.SafeSelectNodes("//p | //div")) //you could add more tags here
    {
    ...
    foreach (var node in doc.DocumentNode.SafeSelectNodes("//br")) {
