...
namespace PodNoms.Common.Services.Minifier;
...
public static class StreamReaderExtension {
  ...
  public static string ReArrangeDeclarations(string fileContents) {
    ...
    var declarations = new Dictionary<string, bool>();
  ...
  private static string ReArrangeDeclaration(string fileContents, string declaration, bool bringToTop) {
    ...
    var matches = Regex.Matches(fileContents, declaration);

    // Loop through the matches
    var alreadyMatched = 0;
    foreach (Match match in matches) {
      var position = declaration.Length;
      var declarationPosition = match.Index;
      ...
      while (declarationPosition >= 0) {
        ...
        var substring = fileContents.Substring(declarationPosition, position);

        // Check if it contains a whitespace at the end
        if (!substring.EndsWith(", ") && (substring.EndsWith(" ")
                                          || (substring.EndsWith(">") &&
                                              fileContents.Substring(declarationPosition + position - 1, 2) != ">>"))) {
          if (bringToTop) {
  ...
  public static string RemoveJavaScriptComments(string javaScriptComments) {
    ...
    var extractScripts = new Regex(@"<script[^>]*>[\s\S]*?</script>");
    ...
    foreach (Match match in extractScripts.Matches(javaScriptComments)) {
      ...
      javaScriptComments =
        javaScriptComments.Replace(scriptBlock, Regex.Replace(scriptBlock, @"[^:|""|']//(.*?)\r?\n", ""));
    }
  ...
  public static string EnsureMaxLength(string htmlContents, Features features) {
    if (features.MaxLength > 0) {
      var htmlLength = htmlContents.Length;
      var currentMaxLength = features.MaxLength;
      int position;
