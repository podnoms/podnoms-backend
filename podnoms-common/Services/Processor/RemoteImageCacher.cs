    ...
    try {
      ...
      if (!extension.Equals("jpg")) {
        if (extension.Equals("webp")) {
          (sourceFile, extension) = await ImageUtils.ConvertFileFromWebp(sourceFile, sourceFile);
        } else {
          (sourceFile, extension) = await ImageUtils.ConvertFile(sourceFile, sourceFile);
        }
      }
