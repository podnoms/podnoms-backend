  ...
  public override async Task<bool> Execute(PerformContext context) {
    ...
    foreach (var entry in entries) {
      try {
        ...
        if (!await ExecuteForEntry(entry, file, false, context)) {
          continue;
        }
  ...
  public async Task<bool> ExecuteForEntry(PodcastEntry entry, string localFile,
    ...
    try {
      ...
      _tagger.CreateTags(
        ...
        $"Copyright © {DateTime.Now.Year} {entry.Podcast.AppUser.GetBestGuessName()}",
      ...
      LogError($"Error tagging entry: {e.Message}");
    }
